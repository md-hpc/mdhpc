...
int main(int argc, char** argv) {
    ...
    // Read input file
    std::vector<int, aligned_allocator<uint8_t>> input_data;
    std::ifstream file(inputFile);
    std::string line;
    while (std::getline(file, line)) {
        if (line.length() != 64) {
            std::cerr << "Invalid line length in input file." << std::endl;
            return EXIT_FAILURE;
        ...
        for (size_t i = 0; i < 64; i += 2) {
            input_data.push_back(std::stoi(line.substr(i, 2), nullptr, 16));
        }
    }
    file.close();
    ...
    std::vector<int, aligned_allocator<uint8_t> > hw_results(size);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            // Creating Kernel
            OCL_CHECK(err, krnl_myadder1 = cl::Kernel(program, "MD_RL", &err));
    ...
    OCL_CHECK(err, cl::Buffer buffer_input(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, vector_size_bytes, input_data.data(),
                                           &err));
