...
timeout = 0
#while (int(ol_w0_md.register_map.iter_cnt) < iter_target):
#    timeout += 1
#    if timeout > 5:
...
#start_recv(s2mm_buf_w0, "dump/recv_w0_bank_.txt")
...
ol_w0_md.register_map.read_ctrl = 0
for i in rcvd_list:
    print([struct.unpack('!f', struct.pack('!I', i[0]))[0],struct.unpack('!f', struct.pack('!I', i[1]))[0],struct.unpack('!f', struct.pack('!I', i[2]))[0]])
